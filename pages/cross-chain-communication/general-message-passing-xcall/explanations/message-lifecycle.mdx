...
6. If an error occurs during execution, and rollback data was included in the original call, an error handling process is initiated, sending a `ResponseMessage` Event back to the source chain for state rollback operations. For more information, see [Troubleshooting](/cross-chain-communication/general-message-passing-xcall/guides/error-handling).

In this process, the dApp developer handles [initiating the cross-chain call](/cross-chain-communication/general-message-passing-xcall/quickstart/sending-a-message#sending-a-message), [responding to events](/cross-chain-communication/general-message-passing-xcall/quickstart/sending-a-message#fetching-event-in-source-chain), and [errors](/cross-chain-communication/general-message-passing-xcall/guides/error-handling).

`sendCallMessage` is a `Payable` method and requires a fee. For more information, see [Handling Fees](/cross-chain-communication/general-message-passing-xcall/explanations/fees).

## Full message lifecycle

For the full message lifecycle, please see the [xCall specification](https://github.com/icon-project/xcall-multi/blob/main/docs/adr/xcall.md).
