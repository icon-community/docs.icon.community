...
Apps use ICON to operate cross-chain seamlessly, build momentum and gain a solid reputation. ICON's Cross-Chain Framework helps to simplify cross-chain development with the easy-to-use ICON General Message Passing (xCall) and connections to secure bridging protocols.
...
## ICON General Message Passing (xCall)
...
Additionally, ICON GMP offers Multi Protocol Verification for implementations that wish to access further assurances for their transactions. This allows a message to be sent over additional protocols, aggregating the results of messages sent to provide a layer of redundancy for the transaction and ensuring its delivery to the destination.
