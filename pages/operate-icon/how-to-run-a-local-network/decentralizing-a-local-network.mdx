...
The process of registering a node as a validator requires signing a transaction with the [registerPRep method](https://github.com/icon-project/goloop/blob/master/doc/icon_chainscore_api.md#registerprep), paying _2000_ ICX and sending a JSON formatted data with the validator information in the following format:
...
* Sign a transaction with each validator wallet calling the [setBonderList](https://github.com/icon-project/goloop/blob/master/doc/icon_chainscore_api.md#setbonderlist) method of the governance contract, to setup which wallets can place bonds for the validator.
* Place the bond with the wallet that was previously whitelisted in the previous step by signing a transaction calling the [setBond](https://github.com/icon-project/goloop/blob/master/doc/icon_chainscore_api.md#setbond) method of the governance wallet.
