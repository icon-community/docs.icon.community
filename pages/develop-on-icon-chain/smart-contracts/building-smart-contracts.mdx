...
The smart contract will be associated with an _Application Binary Interface (ABI)_, which is a markup list of the public-facing, callable endpoints of the contract. A web application, such as a [Javascript](https://github.com/icon-project/icon-sdk-js) client or a [Python](https://github.com/icon-project/icon-sdk-python) service, can read the ABI so that the application can make calls to interact with the smart contract.

You can get an ICON smart contract's ABI by using the [`icx_getScoreApi`](https://github.com/icon-project/goloop/blob/master/doc/jsonrpc_v3.md#icx_getscoreapi) function from the JSON-RPC API. See the ABI specification listed there in the _**Returns**_ descriptor.
