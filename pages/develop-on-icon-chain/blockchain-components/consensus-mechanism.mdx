...
ICON uses a form of the Proof-of-Stake algorithm developed by [Daniel Larimer](https://bitcoinwiki.org/wiki/dan-larimer) called [_Delegated Proof-of-Stake_](https://icon.community/glossary/delegated-proof-of-stake/). The key distinction between the two is that the _delegated_ version does not let any group join the pool of [validator nodes](/operate-icon/network/validator-nodes). The only valid validators are voted in by the community and are known as [delegates](/operate-icon/governance/delegates). They are users who are voted in by the community because they are seen as responsible and reliable for the purpose of processing transactions and validating blocks to add to the blockchain.
...
1. Process the next group of [transactions](/develop-on-icon-chain/blockchain-components/transactions)
2. Perform a hash on the resulting information
3. Store those hashes in a [block](/develop-on-icon-chain/blockchain-components/blocks)
4. Pass that block to the other validators in the ICON [network](/develop-on-icon-chain/networks)
5. Receiving the votes from the other validators on whether the block is valid or not

If the block is valid, then it is added to the blockchain and the validators are [rewarded](/operate-icon/economics/rewards-and-penalties) appropriately. If that block is invalid, then another validator is chosen, and the validators are penalized appropriately. If a bad block is voted as valid, then the validator who staked their ICX by voting to add that block could potentially lose all of their stake if the other validators eventually determine that the block is in fact invalid.
