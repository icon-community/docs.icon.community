...
The three main categories of ways to perform a transaction are as follows: through the [Hana wallet](https://icon.community/hanawallet/), through a decentralized application (dApp), or via the [ICON JSON-RPC API](https://github.com/icon-project/goloop/blob/master/doc/jsonrpc_v3.md).

To use Hana wallet, you must first [create an account](/develop-on-icon-chain/blockchain-components/accounts#creating-an-account). Then, open the wallet via web browser and perform a transaction.
...
There are two main ways to query a transaction: through the [Tracker](https://tracker.icon.community) or through the JSON-RPC API available from an [API Endpoint](/develop-on-icon-chain/networks).
...
If you get the transaction hash as a response to sending a transaction to the ICON network, this means that your transaction has passed the validation. ICON network returns the transaction hash only when a transaction is valid. For specific information about this process, see the JSON-RPC APIs `icx_sendTransaction` section of the [ICON JSON-RPC API v3 Specification](https://github.com/icon-project/goloop/blob/master/doc/jsonrpc_v3.md) documentation.

**Check the Transaction Confirmation using the JSON-RPC API**

You can check if a transaction is confirmed or pending using `icx_getTransactionResult` JSON-RPC API. "Pending" means that a transaction is in the transaction pool or block generation stage (except in the case of consensus failure on the block, it can be temporarily marked as "pending"). For specific information about this process, see the JSON-RPC APIs `icx_getTransactionResult` section of the [ICON JSON-RPC API v3 Specification](https://github.com/icon-project/goloop/blob/master/doc/jsonrpc_v3.md) documentation.
...
Fees are incurred when executing a smart contract on the ICON network, and the fees are determined based on the total amount of resources used to execute the transaction. [STEP](/operate-icon/economics/step) is the unit of the resource-usage measurement.
